...
for i in range(162, 236):
    ...
    try:
        with sync_playwright() as p:
            ...
            page.goto(f"https://www.realtor.com/realestateagents/houston_tx/rep-seller/photo-1/pg-{i}", timeout=0)
            ...
            for agent in agents:
                ...
                with open('Houston.csv', 'a+', encoding='utf-8', newline='') as file:
                    writer = csv.writer(file)
                    if file.tell() == 0:  # Check if file is empty and write header
                        writer.writerow(header)
                    writer.writerow([agent['name'], agent['phone'], agent['company'], 'Houston'])
