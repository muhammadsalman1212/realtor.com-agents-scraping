...
for i in range(1, 134):
    ...
    try:
        with sync_playwright() as p:
            ...
            page.goto(f"https://www.realtor.com/realestateagents/san-antonio_tx/rep-seller/photo-1/pg-{i}", timeout=0)
            ...
            for agent in agents:
                ...
                with open('San-Antonio.csv', 'a+', encoding='utf-8', newline='') as file:
                    writer = csv.writer(file)
                    if file.tell() == 0:  # Check if file is empty and write header
                        writer.writerow(header)
                    writer.writerow([agent['name'], agent['phone'], agent['company'], 'San-Antonio'])
